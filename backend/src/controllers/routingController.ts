...
export class RoutingController {
    static async list(req: Request, res: Response, next: NextFunction) {
        try {
            ...
            const items = await prisma.routing.findMany({ where: { userId }, include: { finishedGoodItem: true } });
            res.json(items);
        } catch (e) { next(e); }
    }
    static async get(req: Request, res: Response, next: NextFunction) {
        try {
            const userId = (req as any).user.userId;
            const item = await prisma.routing.findUnique({ where: { id: parseInt(req.params.id) }, include: { finishedGoodItem: true, operations: { include: { workCenter: true } } } });
